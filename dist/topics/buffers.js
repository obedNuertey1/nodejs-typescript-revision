export default () => {
    ...
    console.log(buf);
    //Method2
    let buf2 = Buffer.from([10, 20, 30, 40, 50]);
    console.log(buf2);
    //Method 3
    let buf1 = Buffer.from("Simply Easy Learning", "utf8");
    console.log(buf1);
    //Writing to Buffers
    const bufObj = Buffer.alloc(256);
    console.log(bufObj);
    let len = bufObj.write("Simply Easy Learning");
    console.log("Octets written : " + len);
    console.log(bufObj);
    //Reading from buffers
    console.log("\n\n**** READING FROM BUFFERS ***********");
    const bufObj2 = Buffer.alloc(26);
    for (let i = 0; i < 26; i++) {
        bufObj2[i] = i + 97;
    }
    let strObj = bufObj2.toString('ascii');
    console.log(strObj);
    ...
    console.log("\n\n********* CONVERT BUFFER TO JSON **************");
    const bufObj3 = Buffer.from("Simply Easy Learning");
    let json = bufObj3.toJSON();
    console.log(json);
    //Concatenate buffers
    //console.log("\n\n*********** Concatenate Buffer ****************");
    ...
    let isBuffEncoding = Buffer.isEncoding("utf8");
    console.log(`isBuffEncoding is ${isBuffEncoding}`);
    let isBuffObj = Buffer.isBuffer(buffer1);
